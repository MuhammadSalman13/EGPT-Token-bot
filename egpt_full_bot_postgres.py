...
# -------- الاتصال بقاعدة البيانات عبر Supabase Transaction Pooler ----------
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    row = c.fetchone()
    if not row:
        await update.message.reply_text("❌ Error fetching user data.", reply_markup=WELCOME_KEYBOARD)
        return
    ...
    if text == "💰 Earn":
        ...
        conn.commit()
        await update.message.reply_text(f"Your withdraw request of {egpt_balance:.2f} EGPT has been submitted and is pending approval.", reply_markup=WELCOME_KEYBOARD)
