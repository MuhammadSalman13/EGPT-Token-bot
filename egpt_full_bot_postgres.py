...
if not BOT_TOKEN:
    raise Exception("❌ BOT_TOKEN is missing! Please set it in Railway Environment Variables.")

if not DATABASE_URL:
    raise Exception("❌ DATABASE_URL is missing! Please set it in Railway Environment Variables.")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "💰 Earn":
        ...
        if not wallet_address:
            ...
            return
        c.execute("INSERT INTO withdrawals(user_id, amount_egpt, wallet_address, request_date) VALUES (%s,%s,%s,%s)", (user_id, egpt_balance, wallet_address, now))
        conn.commit()
