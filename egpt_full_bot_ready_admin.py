...
ADMIN_IDS = [5808513261]  # Telegram ID بتاعك كأدمن
...
async def admin_users(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    rows = c.fetchall()
    if not rows:
        await update.message.reply_text("No users found.", reply_markup=WELCOME_KEYBOARD)
        return
    msg = "All Users:\n"
    for r in rows:
        msg += f"User ID: {r[0]}, Micro-Tokens: {r[1]}, Invites: {r[2]}, Wallet: {r[3]}\n"
...
def main():
    ...
    app.add_handler(CommandHandler("users", admin_users))      # أمر الأدمن لعرض المستخدمين
    app.add_handler(CommandHandler("pending", admin_pending))  # أمر الأدمن لعرض طلبات السحب
