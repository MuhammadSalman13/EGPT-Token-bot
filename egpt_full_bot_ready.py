...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'tap':
        ...
        conn.commit()
        await query.message.reply_text(f"You earned {MICRO_TOKENS_PER_TAP} Micro-Tokens! Total today: {daily_tokens}", reply_markup=get_keyboard())
        ...
        if last_checkin == now:
            await query.message.reply_text("You already did your daily check-in today!", reply_markup=get_keyboard())
            return
        ...
        conn.commit()
        await query.message.reply_text(f"Daily Check-in done! You earned {MICRO_TOKENS_CHECKIN} Micro-Tokens.\nTotal: {micro_tokens}", reply_markup=get_keyboard())

    elif query.data == 'set_wallet':
        await query.message.reply_text("Please send me your USDT wallet address on BSC network now:", reply_markup=get_keyboard())

    elif query.data == 'withdraw':
        egpt_balance = micro_tokens / MICRO_TOKENS_TO_EGPT * 10
        if egpt_balance < WITHDRAW_MIN_EGPT:
            await query.message.reply_text(f"Minimum withdraw is {WITHDRAW_MIN_EGPT} EGPT. Your balance: {egpt_balance:.2f} EGPT", reply_markup=get_keyboard())
            return
        if invited_count < INVITE_REQUIREMENT:
            await query.message.reply_text(f"You need at least {INVITE_REQUIREMENT} invited friends to withdraw.\nCurrent: {invited_count}", reply_markup=get_keyboard())
            return
        if not wallet_address:
            await query.message.reply_text("Please set your wallet first using 'Set Wallet'.", reply_markup=get_keyboard())
            return
        ...
        conn.commit()
        await query.message.reply_text(f"Your withdraw request of {egpt_balance:.2f} EGPT has been submitted and is pending approval by admin.", reply_markup=get_keyboard())
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text.startswith("0x") and len(text) == 42:
        ...
        conn.commit()
        await update.message.reply_text("Wallet address saved successfully!", reply_markup=get_keyboard())
    else:
        await update.message.reply_text("Invalid wallet address. Make sure it starts with 0x and is 42 characters long.", reply_markup=get_keyboard())
