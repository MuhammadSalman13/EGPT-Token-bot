# egpt_full_bot_ready.py  (النسخة النهائية - نفس الاسم الأصلي)
import sqlite3
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "🔧 Admin" and user_id in ADMIN_IDS:
        ...
        c.execute("SELECT SUM(micro_tokens) FROM users"); total_balance = c.fetchone()[0] or 0
        c.execute("SELECT user_id, micro_tokens FROM users ORDER BY micro_tokens DESC LIMIT 5"); top_users = c.fetchall()
        c.execute("SELECT id, user_id, amount_egpt, status FROM withdrawals ORDER BY id DESC LIMIT 5"); withdraws = c.fetchall()
        
        msg = f"🔧 لوحة التحكم:
👥 المستخدمين: {total_users}
💰 الرصيد الكلي: {total_balance:,}
...
💸 آخر 5 سحوبات:
"
        for w in withdraws: msg += f"ID {w[0]}: User {w[1]} ({w[2]:.1f}EGPT) {w[3]}
"
    ...
    if text == "📋 Pending" and user_id in ADMIN_IDS:
        ...
        msg = "المعلقة:
" + "
".join([f"{r[0]}: {r[2]:.1f}EGPT" for r in rows]) if rows else "لا يوجد"
    ...
    if text == "💰 Earn":
        ...
        if daily_tokens > MICRO_TOKENS_DAILY_MAX: daily_tokens = MICRO_TOKENS_DAILY_MAX
...
async def admin_pending(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = "المعلقة:
" + "
".join([f"{r[0]}: User{r[1]} {r[2]:.1f}EGPT" for r in rows]) if rows else "لا يوجد"
