...
BOT_TOKEN = "8594208349:AAF8BqZUWs9TCOqo3Lw7Jw3kxva7BBKruX4"
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'tap':
        ...
        c.execute("UPDATE users SET waiting_wallet=1 WHERE user_id=?", (user_id,))
        conn.commit()
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    c.execute("SELECT waiting_wallet FROM users WHERE user_id=?", (user_id,))
    row = c.fetchone()
    waiting_wallet = row[0] if row else 0

    if waiting_wallet:
        if text.startswith("0x") and len(text) == 42:
            c.execute("UPDATE users SET wallet_address=?, waiting_wallet=0 WHERE user_id=?", (text, user_id))
            conn.commit()
