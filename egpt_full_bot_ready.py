...
async def admin_list_users(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for r in rows:
        msg += (f"User ID: {r[0]}, Micro-Tokens: {r[1]}, Last Tap: {r[2]}, "
                f"Last Check-in: {r[3]}, Invited: {r[4]}, Wallet: {r[5]}\n")
    await update.message.reply_text(msg, reply_markup=WELCOME_KEYBOARD)
...
def main():
    ...
    app.add_handler(CommandHandler("users", admin_list_users))  # <- الأمر الجديد
