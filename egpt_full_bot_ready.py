...
WELCOME_TEXT = (
    ...
    "Use the buttons below to earn, check balance, invite friends, and withdraw."
)
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'tap':
        ...
        conn.commit()
        await query.edit_message_text(f"Daily Check-in done! You earned {MICRO_TOKENS_CHECKIN} Micro-Tokens.\nTotal: {micro_tokens}", reply_markup=get_main_menu())

    elif query.data == 'set_wallet':
        await query.edit_message_text("Please send me your USDT wallet address on BSC network now:", reply_markup=get_main_menu())

    elif query.data == 'withdraw':
        egpt_balance = micro_tokens / MICRO_TOKENS_TO_EGPT * 10
        if egpt_balance < WITHDRAW_MIN_EGPT:
            await query.edit_message_text(f"Minimum withdraw is {WITHDRAW_MIN_EGPT} EGPT. Your balance: {egpt_balance:.2f} EGPT", reply_markup=get_main_menu())
            return
        if invited_count < INVITE_REQUIREMENT:
            await query.edit_message_text(f"You need at least {INVITE_REQUIREMENT} invited friends to withdraw.\nCurrent: {invited_count}", reply_markup=get_main_menu())
            return
        if not wallet_address:
            await query.edit_message_text("Please set your wallet first using 'Set Wallet'.", reply_markup=get_main_menu())
            return
        ...
        conn.commit()
        await query.edit_message_text(f"Your withdraw request of {egpt_balance:.2f} EGPT has been submitted and is pending approval by admin.", reply_markup=get_main_menu())
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text.startswith("0x") and len(text) == 42:
        ...
        conn.commit()
        await update.message.reply_text("Wallet address saved successfully!", reply_markup=get_main_menu())
    else:
        await update.message.reply_text("Invalid wallet address. Make sure it starts with 0x and is 42 characters long.", reply_markup=get_main_menu())
...
async def admin_pending(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    rows = c.fetchall()
    if not rows:
        await update.message.reply_text("No pending withdrawals.", reply_markup=get_main_menu())
        return
...
async def admin_approve(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    args = context.args
    if not args:
        await update.message.reply_text("Usage: /approve <withdraw_id>", reply_markup=get_main_menu())
        return
    withdraw_id = args[0]
    c.execute("SELECT user_id, amount_egpt FROM withdrawals WHERE id=? AND status='pending'", (withdraw_id,))
    row = c.fetchone()
    if not row:
        await update.message.reply_text("Withdraw request not found.", reply_markup=get_main_menu())
        return
    uid, amount = row
    ...
    conn.commit()
    await update.message.reply_text(f"Withdrawal {withdraw_id} approved. User {uid} balance updated.", reply_markup=get_main_menu())
...
async def admin_reject(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    args = context.args
    if not args:
        await update.message.reply_text("Usage: /reject <withdraw_id>", reply_markup=get_main_menu())
        return
    withdraw_id = args[0]
    c.execute("UPDATE withdrawals SET status='rejected' WHERE id=?", (withdraw_id,))
    conn.commit()
    await update.message.reply_text(f"Withdrawal {withdraw_id} rejected.", reply_markup=get_main_menu())

# -------- تشغيل البوت ----------
def main():
    TOKEN = "هنا تحط توكن البوت"
