...
ADMIN_IDS = [123456789]  # Telegram ID بتاعك
BOT_USERNAME = "EGPTCOINSBot"

# -------- قائمة الترحيب الثابتة (menu buttons) ----------
WELCOME_KEYBOARD = InlineKeyboardMarkup([
    [InlineKeyboardButton("💰 Earn", callback_data='tap')],
    [InlineKeyboardButton("📊 Balance", callback_data='balance')],
    [InlineKeyboardButton("👥 Referrals", callback_data='invite')],
...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'tap':
        ...
        conn.commit()
        await query.edit_message_text(f"You earned {MICRO_TOKENS_PER_TAP} Micro-Tokens! Total today: {daily_tokens}", reply_markup=WELCOME_KEYBOARD)
        ...
        if last_checkin == now:
            await query.edit_message_text("You already did your daily check-in today!", reply_markup=WELCOME_KEYBOARD)
            return
        ...
        await query.edit_message_text("Please send me your USDT wallet address on BSC network now:", reply_markup=WELCOME_KEYBOARD)

    elif query.data == 'withdraw':
        egpt_balance = micro_tokens / MICRO_TOKENS_TO_EGPT * 10
        if egpt_balance < WITHDRAW_MIN_EGPT:
            await query.edit_message_text(f"Minimum withdraw is {WITHDRAW_MIN_EGPT} EGPT. Your balance: {egpt_balance:.2f} EGPT", reply_markup=WELCOME_KEYBOARD)
            return
        ...
        if not wallet_address:
            await query.edit_message_text("Please set your wallet first using 'Set Wallet'.", reply_markup=WELCOME_KEYBOARD)
            return
        ...
        conn.commit()
        await query.edit_message_text(f"Your withdraw request of {egpt_balance:.2f} EGPT has been submitted and is pending approval by admin.", reply_markup=WELCOME_KEYBOARD)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text.startswith("0x") and len(text) == 42:
        ...
        await update.message.reply_text("Invalid wallet address. Make sure it starts with 0x and is 42 characters long.", reply_markup=WELCOME_KEYBOARD)

# -------- أوامر Admin ----------
async def admin_users(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.from_user.id not in ADMIN_IDS:
        return
    c.execute("SELECT user_id, micro_tokens, invited_count, wallet_address FROM users")
    rows = c.fetchall()
    msg = "Users:\n"
    for r in rows:
        msg += f"User: {r[0]}, Micro: {r[1]}, Invites: {r[2]}, Wallet: {r[3]}\n"
...
# -------- تشغيل البوت ----------
def main():
    TOKEN = "8594208349:AAF8BqZUWs9TCOqo3Lw7Jw3kxva7BBKruX4"  # ضع التوكن هنا
    app = ApplicationBuilder().token(TOKEN).build()

    app.add_handler(CommandHandler("start", start))
    app.add_handler(CommandHandler("users", admin_users))  # أمر لعرض كل المستخدمين
