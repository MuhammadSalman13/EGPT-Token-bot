...
async def admin_users(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id not in ADMIN_IDS:
        await update.message.reply_text("You are not an admin!")
        return
    c.execute("SELECT user_id, micro_tokens, last_tap, last_checkin, invited_count, wallet_address FROM users")
    rows = c.fetchall()
    msg = "All Users:\n"
    for r in rows:
        msg += f"User: {r[0]}, Micro: {r[1]}, Last Tap: {r[2]}, Last Check-in: {r[3]}, Invited: {r[4]}, Wallet: {r[5]}\n"
    await update.message.reply_text(msg, reply_markup=WELCOME_KEYBOARD)

# -------- تشغيل البوت ----------
def main():
    TOKEN = "8594208349:AAF8BqZUWs9TCOqo3Lw7Jw3kxva7BBKruX4"  # ضع التوكن الجديد هنا
